...
def build_prompt(query, retrieved_results):
    """
    Constructs a final prompt that combines retrieved context with the user query.
    This prompt explicitly instructs the model to provide a short answer that includes:
      - A concise NN code snippet.
...
llm_generator = pipeline("text-generation", model=model, tokenizer=tokenizer, max_new_tokens=2000)
